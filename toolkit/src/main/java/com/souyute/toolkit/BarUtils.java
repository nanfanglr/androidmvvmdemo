...
public final class BarUtils {
     ...
     * @return the action bar's height
     */
    public static int getActionBarHeight(Application app) {
        TypedValue tv = new TypedValue();
        if (app.getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
            return TypedValue.complexToDimensionPixelSize(
                    tv.data, app.getResources().getDisplayMetrics()
            );
        }
        return 0;
    }
    ...
    public static void setNotificationBarVisibility(final boolean isVisible, Application app) {
        String methodName;
        if (isVisible) {
        ...
        invokePanels(methodName, app);
    }

    private static void invokePanels(final String methodName, Application app) {
        try {
            @SuppressLint("WrongConstant")
            Object service = app.getSystemService("statusbar");
